...
#[cfg(feature = "async")]
fn dump_vsock_message(direction: &str, header: &MessageHeader, payload: &[u8]) {
    println!(
        "vsock {} ttrpc header={{ length: {}, stream_id: {}, type: {}, flags: {} }} bytes={} text={:?} raw={:?}",
        direction,
        header.length,
        header.stream_id,
        header.type_,
...
impl GenMessage {
    ...
    pub async fn write_to(
        ...
        dump_vsock_message("send", &self.header, &self.payload);
        self.header
            .write_to(&mut writer)
            .await
            .map_err(|e| Error::Socket(e.to_string()))?;
    ...
    pub async fn read_from(
        ...
        dump_vsock_message("recv", &header, &content);
